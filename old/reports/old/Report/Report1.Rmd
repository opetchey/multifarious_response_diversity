...
p1E1 <- draw(m1, rug = FALSE) &
  ...
  labs(tag = "(a)") & xlab("Temperature (k)") & ylab("Salinity (ppt)") & 
   labs(title = "te(Temperature, Salinity)") & theme_classic(base_size = 20) 
p1E1 
```
...
p2 <- sm_E1|>
  ...
  geom_line() +
  labs(x = "Temperature", y = "Partial effect Temperature", title = "te(Temperature,Salinity)",
...
p3 <- pd_E1 |>
    draw() &
    labs(x = "Temperature", y = "Partial derivative with respest to Temperature", title = "te(Temperature,Salinity)", caption = cap, tag = "(c)") & theme_classic(base_size = 20)  &
...
p4 <- draw(m1, rug = FALSE) &
  ...
  labs(tag = "(a)") & xlab("Temperature (k)") & ylab("Salinity (ppt)") & 
   labs(title = "te(Temperature, Salinity)", fill = "Growth") & theme_classic(base_size = 20) 
p4+ guides(fill=guide_legend(title="Growth \n Rate"))
...
p5<- sm_E2 |>
  ...
  geom_ribbon(aes(ymin = lower_ci, ymax = upper_ci), alpha = 0.2) +
  geom_line() +
  labs(x = "Salinity", y = "Partial effect Salinity", title = "te(Temperature,Salinity)",
...
p6 <- pd_E2 |>
  draw() +
  labs(x = "Salinity", y = "Partial derivative with respest to Salinity", title = "te(Temperature,Salinity)", caption = cap, tag = "(e)") & theme_classic(base_size = 20)  &
   geom_hline(yintercept = 0, size = 1, linetype = "dashed")
...
UnkownRdiv %>% 
  ggplot(aes(x = community, y = divergence, fill = community)) +
