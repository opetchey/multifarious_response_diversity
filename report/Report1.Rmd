...
To explore this hypothesis, we calculate now response diversity for two communities composed of 3 spp in 4 different cases:
...
<!-- (ref:spp-surf) Species' response surfaces. (a) is species 1, (b) is species 2, and (c) is species 3.  -->
<!-- ```{r spp-surf, fig.cap='(ref:spp-surf)', fig.align="center", fig.height=3, fig.width=12} -->

<!-- # Plot spp surfaces and selecting spp with similar magnitude of responses, so we will see them later in the RD plot -->
<!-- sp1m <- nested_gams_no_inter$gams[[1]] -->
<!-- sp1 <- draw(sp1m, rug = FALSE) & -->
<!--       labs(tag = "(a)") -->
...
<!-- p_spp_surf <- wrap_plots(sp1, sp3, sp10) -->
<!-- p_spp_surf -->


<!-- p_spp_surf <- wrap_plots(sp2, sp5, sp13) -->
<!-- p_spp_surf -->
<!-- ``` -->
...
#community 1 
m_list_no <- (nested_gams_no_inter$gams[c(4, 6, 11)])

my_spp_names_no <- (nested_gams_no_inter$species[c(4, 6, 11)])
...
#community 2
m_list_no2 <- (nested_gams_no_inter$gams[c(2, 5, 13)])

my_spp_names_no2 <- (nested_gams_no_inter$species[c(2, 5, 13)])


(pd_list_no2 <- modify_depth(m_list_no2, 1, ~ get_partials(., refs2)))


# from list to tibble
(pd_spp_no2 <- tibble(
  E1_ref = map(pd_list_no2, "E1"),
  E2_ref = map(pd_list_no2, "E2"),
  pd_E1 = map(pd_list_no2, "pd_E1"),
  pd_E2 = map(pd_list_no2, "pd_E2")) %>% 
    dplyr::mutate(sp = my_spp_names_no2) %>% 
    relocate(sp, E1_ref, E2_ref, pd_E1, pd_E2) %>% 
    unnest(E1_ref, E2_ref, pd_E1, pd_E2))
...
radius <- 1
num_arrows <- 20
pd_spp_no2 <- tibble(angle = rep(seq(0, 2*pi, length = num_arrows), nrow(pd_spp_no2))) %>%
  mutate(E1_ref = rep(pd_spp_no2$E1_ref, each = num_arrows),
         E2_ref = rep(pd_spp_no2$E2_ref, each = num_arrows)) %>%
  full_join(pd_spp_no2) %>%
  mutate(E1 = cos(angle) * radius,
         E2 = sin(angle) * radius,
         dir_deriv = E1 * pd_E1 + E2 * pd_E2,
         unit_vec_mag = sqrt(E1^2 + E2^2))
...
(tmp_rd_multi_no2 <- dd2_no2 %>% dplyr::select(E1_ref, E2_ref, sp, mean_dd) %>% 
  group_by(E1_ref, E2_ref, sp) %>% nest() %>% 
  mutate(dir_d = map(data, "mean_dd")) %>% 
  unnest(dir_d) %>% 
  dplyr::select(-data) %>% 
...
rd_multi_no$rdiv <- apply(rd_multi_no[,c(3:5)], 1, resp_div, sign_sens = F)
rd_multi_no$sign <- apply(rd_multi_no[,c(3:5)], 1, resp_div, sign_sens = T)
rd_multi_no$Med<-median(rd_multi_no$rdiv)
rd_multi_no

# community 2 
(rd_multi_no2 <- tmp_rd_multi_no2 %>% pivot_wider(names_from = sp, values_from = dir_d))


rd_multi_no2$rdiv <- apply(rd_multi_no[,c(3:5)], 1, resp_div, sign_sens = F)
rd_multi_no2$sign <- apply(rd_multi_no[,c(3:5)], 1, resp_div, sign_sens = T)
rd_multi_no2$Med<-median(rd_multi_no$rdiv)
rd_multi_no2
```
...
refs_ts <- mvrnorm(n = 15,        #  specifies the sample size
                 ...
                 Sigma = matrix(c( 0.0001, 0.000,
                                  0.000,  0.0001), #  specifies the correlation matrix
                                nrow = 2))
...
#community 2
# get partial derivatives
(pd_list_no2 <- modify_depth(m_list_no2, 1, ~ get_partials(., refs_ts)))
# from list to tibble
(pd_spp_no2 <- tibble(
  E1_ref = map(pd_list_no2, "E1"),
  E2_ref = map(pd_list_no2, "E2"),
  ...
  pd_E2 = map(pd_list_no2, "pd_E2")) %>%
    ...
    unnest(E1_ref, E2_ref, pd_E1, pd_E2))
# add time
(pd_spp_no2 <-cbind(pd_spp_no2, refs_ts$time) %>% 
    dplyr::rename(time = "refs_ts$time"))

# calculation next value for directional derivatives, and get directional derivatives
(pd_spp_no2 <- pd_spp_no2 %>% transform( nxt_value_E1 = c(E1_ref[-1], NA)) %>%
    transform(nxt_value_E2 = c(E2_ref[-1], NA)) %>%
    dplyr::mutate(del_E1 = nxt_value_E1 - E1_ref,
                  del_E2 = nxt_value_E2 - E2_ref,
                  unit_vec_mag =  sqrt(del_E1^2 + del_E2^2),
...
# community 2 
# reduce the dataframe and keep only what we need
red_spp2 <- pd_spp_no2 %>%  dplyr::select(sp, time, E1_ref, E2_ref, dir_deriv)
# from long to wide
...
(ref:RDindependentplot) Directional derivatives and response diversity with known direction of env change. E1 and E2 change independently over time.
a and b: Species directional derivatives over time. c and d: Response diversity measured as similarity-based diversity metric. e and : Response diversity measured as divergence (sign sensitive). 


```{r RDindependentplot, fig.cap='(ref:RDindependentplot)', fig.align="center", fig.height=6, fig.width=12}
# community 1 
...
random_ev_change2 <- dd_plot2 / rdiv_plot2 / sing_plot2
#random_ev_change2
...
refs_neg <- mvrnorm(n = 15,        #  specifies the sample size
                 mu = c(mean(E1_series), mean(E2_series)), #specifies the mean values of each column
                 Sigma = matrix(c( 1, -1,
                                  -1,  1), #  specifies the correlation matrix
                                nrow = 2))
...
# community 2 
# get partial derivatives
(pd_list_no2 <- modify_depth(m_list_no2, 1, ~ get_partials(., refs_neg)))
# from list to tibble
(pd_spp_no2 <- tibble(
  E1_ref = map(pd_list_no2, "E1"),
  E2_ref = map(pd_list_no2, "E2"),
  ...
  pd_E2 = map(pd_list_no2, "pd_E2")) %>%
    ...
    unnest(E1_ref, E2_ref, pd_E1, pd_E2))
# add time
(pd_spp_no2 <-cbind(pd_spp_no2, refs_neg$time) %>% 
    dplyr::rename(time = "refs_neg$time"))
...
# actual calculation for only the same species used above
rdiv_sim_neg$rdiv<-apply(rdiv_sim_neg[,c(4:6)], 1, resp_div, sign_sens = F)
rdiv_sim_neg$sign<-apply(rdiv_sim_neg[,c(4:6)], 1, resp_div, sign_sens = T)
...
# actual calculation for only the same species used above
rdiv_sim_neg2$rdiv<-apply(rdiv_sim_neg2[,c(4:6)], 1, resp_div, sign_sens = F)
rdiv_sim_neg2$sign<-apply(rdiv_sim_neg2[,c(4:6)], 1, resp_div, sign_sens = T)
...
(ref:RDnegativeplot) Directional derivatives and response diversity with known direction of env change. E1 and E2 change with negative correlation over time.
a and b: Species directional derivatives over time. c and d: Response diversity measured as similarity-based diversity metric. e and : Response diversity measured as divergence (sign sensitive). 


```{r RDnegativeplot, fig.cap='(ref:RDnegativeplot)', fig.align="center", fig.height=6, fig.width=12}
...
sing_plot2 <- ggplot(data = rdiv_sim_neg2, mapping = aes(x = time, y = sign)) +
  ...
  theme_classic() +
   labs(x = "time",y = "Divergence", tag = "f")
neg_ev_change2 <- dd_plot2 / rdiv_plot2 / sing_plot2
#neg_ev_change2
...
refs_pos <- mvrnorm(n = 15,        #  specifies the sample size
                 ...
                 Sigma = matrix(c( 1, 0.8,
                                 0.8,  1), #  specifies the correlation matrix
                                nrow = 2))
...
# community 2 
# get partial derivatives
(pd_list_no2 <- modify_depth(m_list_no2, 1, ~ get_partials(., refs_pos)))
# from list to tibble
(pd_spp_no2 <- tibble(
  E1_ref = map(pd_list_no2, "E1"),
  E2_ref = map(pd_list_no2, "E2"),
  ...
  pd_E2 = map(pd_list_no2, "pd_E2")) %>%
    ...
    unnest(E1_ref, E2_ref, pd_E1, pd_E2))
# add time
(pd_spp_no2 <-cbind(pd_spp_no2, refs_pos$time) %>% 
    dplyr::rename(time = "refs_pos$time"))
...
# actual calculation for only the same species used above
rdiv_sim_pos$rdiv<-apply(rdiv_sim_pos[,c(4:6)], 1, resp_div, sign_sens = F)
rdiv_sim_pos$sign<-apply(rdiv_sim_pos[,c(4:6)], 1, resp_div, sign_sens = T)
...
# actual calculation for only the same species used above
rdiv_sim_pos2$rdiv<-apply(rdiv_sim_pos2[,c(4:6)], 1, resp_div, sign_sens = F)
rdiv_sim_pos2$sign<-apply(rdiv_sim_pos2[,c(4:6)], 1, resp_div, sign_sens = T)
...
(ref:RDpositiveplot) Directional derivatives and response diversity with known direction of env change for community 1 and 2. E1 and E2 change with negative correlation over time.
a and b: Species directional derivatives over time. c and d: Response diversity measured as similarity-based diversity metric. e and : Response diversity measured as divergence (sign sensitive). 


```{r RDpositiveplot, fig.cap='(ref:RDpositiveplot)', fig.align="center", fig.height=12, fig.width=12}
# community 1 
...
(ref:plotcorrelations) Correlation types and response diversity.
a and c: correlation types and response diversity measured as dissimilarity in the first derivatives (sign insensitive) for community 1 and 2 respectively. c and d. correlation types and response diversity measured as divergence in the first derivatives (sign sensitive) for community 1 and 2 respectively

```{r plotcorrelations, fig.cap='(ref:plotcorrelations)', fig.align="center", fig.height=8, fig.width=12}

# community 1 
dd1 <- data.frame(rdiv_sim_neg[, c(7:8)]) 
dd1$cor <- "negative"
dd1$community <- 1

dd2 <- data.frame(rdiv_sim_pos[, c(7:8)]) 
dd2$cor <- "positive"
dd2$community <- 1

dd3 <- data.frame(rdiv_sim_nocor[, c(7:8)]) 
dd3$cor <- "nocor"
dd3$community <- 1

# community 2 
dd4 <- data.frame(rdiv_sim_neg2[, c(7:8)]) 
dd4$cor <- "negative"
dd4$community <- 2

dd5 <- data.frame(rdiv_sim_pos2[, c(7:8)]) 
dd5$cor <- "positive"
dd5$community <- 2

dd6 <- data.frame(rdiv_sim_nocor2[, c(7:8)]) 
dd6$cor <- "nocor"
dd6$community <- 2
...
(ref:ciliates) Species responses to the environmental drivers.
a. Species responses to nutrient concentrations. b. Species responses to light intensity. 

```{r ciliates, fig.cap='(ref:ciliates)', fig.align="center", fig.height=10, fig.width=20}
...
(ref:partialderiv-empirical) Response surface of Colpidium. The two solid lines show at which level of nutrients and light each partial derivative is going to be calculated. Not sure we get the gray areas...
...
(ref:RDempiricalplot) Directional derivatives and response diversity with known direction of env change. 
...
```{r RDempiricalplot, fig.cap='(ref:RDempiricalplot)', fig.align="center", fig.height=12, fig.width=10}
...
```{r RD-compositions, fig.cap='(ref:RD-compositions)', fig.align="center", fig.height=15, fig.width=22}
