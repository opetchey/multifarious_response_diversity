...
```{r, echo=FALSE, results='hide'}
# Change this. What we want now is calculating response diversity in each of the 100 directions, and then take the mean of all responses diversity across the surface
# community 1 
radius <- 1
num_arrows <- 100
...
# filter useless stuff
pd_spp_yes <- pd_spp_yes %>% 
  dplyr::select(angle, sp, E1_ref, E2_ref, dir_deriv)


# Convert df into a list based on angle (so direction of env change)

lst_dd <- pd_spp_yes %>% group_by(angle) %>% 
              group_map(~.x)
...
### Response diversity calculation
# dissimilarity
rdiv_2 <- lapply(final_lst, function(x) cbind(x,"rdiv"= apply(x, 1, resp_div, sign_sens = F)))


# divergence
rdiv_2 <- lapply(rdiv_2, function(x) cbind(x,"sign"= apply(x, 1, resp_div, sign_sens = T)))


# from list to tibble
rdiv_2 <- tibble(
  rdiv = map(rdiv_2, "rdiv"),
  sing = map(rdiv_2, "sign")) %>% 
    mutate(points = row_number()) %>% 
    unnest(rdiv, sing)
...
In this case, we calculated response diversity for a community in all possible directions across the surface, which represents in our opinion the most sensible way to measure the absolute response diversity of a specific community (only first 6 rows shown). 
```{r results='asis', message=FALSE, warning=FALSE}

knitr::kable(head(rdiv_1), col.names = gsub("[.]", " ", names(rdiv_1)))

```


[Plotting response diversity - think about how to plot RD when we do not know the direction of env change.
The plot below isn't very clear. Doing the same in 3D would be best]

```{r}
p <-pd_spp_yes %>%  ggplot(aes(x = E1_ref, y = E2_ref)) + theme_bw()

radius <- 4.5
num_arrows <- 20
dd1 <- tibble(angle = rep(seq(0, 2*pi, length = num_arrows), nrow(rdiv_2))) %>% 
  cbind(rdiv_2) %>%  mutate(E1 = cos(angle) * radius,
         E2 = sin(angle) * radius)
rv1 <- p +  geom_segment(data = dd1,
               aes(x = 300, y = 20,
                   xend = 300 + E1, yend = 20 + E2,
                   col = rdiv), alpha = 0.7,
               size=1) +  scale_colour_gradient2(low = muted("blue"),
                         mid = "white",
                         high = muted("red"),
                         midpoint = mean(dd1$rdiv))



p2 <-pd_spp_no %>%  ggplot(aes(x = E1_ref, y = E2_ref)) + theme_bw()

radius <- 4.5
num_arrows <- 20
dd2 <- tibble(angle = rep(seq(0, 2*pi, length = num_arrows), nrow(rdiv_1))) %>% 
  cbind(rdiv_1) %>%  mutate(E1 = cos(angle) * radius,
         E2 = sin(angle) * radius)
rv2 <- p2 +  geom_segment(data = dd2,
               aes(x = 300, y = 20,
                   xend = 300 + E1, yend = 20 + E2,
                   col = rdiv), alpha = 0.7,
               size=1) +  scale_colour_gradient2(low = muted("blue"),
