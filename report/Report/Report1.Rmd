...
(ref:partialderiv-species1) Response surface of sp1. The two solid lines show at which level of E1 and E2 each partial derivative is going to be calculated.
...
### One point
We start showing how directional derivatives can be calculated even when the direction of the environmental change is unknown. 
In this case, we calculate, for a specific point (E1 = 300, E2 = 20), directional derivatives in all directions. 

(ref:one-point-all-directions) Directional derivatives calculated in all possible direction for a specific point on the response surface of sp1. Clearly, the slope of the directional derivative depends on the direction (red positive, blue negative). 
...
(ref:all-points-all-directions) Directional derivatives calculated in all possible direction for all points on the response surface of sp1. Clearly, the slope of the directional derivative depends on the direction (red positive, blue negative). 
...
(ref:all-points-all-directions-sp2) Directional derivatives calculated in all possible direction for all points on the response surface of sp1. Clearly, the slope of the directional derivative depends on the direction (red positive, blue negative). 
...
Now we calculate response diversity for a community composed of 3 spp for 4 different cases:
...
(i) Simulate spp performance curves with the modified Eppley funciton.
...
dd <- species_pars_no_inter %>% dplyr::select(species, E1, E2, rate) %>% 
  dplyr::filter(species == c("s1", "s2", "s3", "s4"))
...
# GAMs
nested_gams_no_inter <- species_pars_no_inter[,c(1,3,4,5)] %>% 
  nest(cols =-species) %>% 
  mutate(
...
m_list_no <- (nested_gams_no_inter$gams[1:4])

my_spp_names_no <- (nested_gams_no_inter$species[1:4])
...
### Unknown direction of the environmental change - Calculation of directional derivatives in all possible directions
```{r, echo=FALSE, results='hide'}
radius <- 1
num_arrows <- 20
...
set.seed(465)
refs_ts <- tibble(sample(E1_series, 15),
                           sample(E2_series, 15)) %>% 
  dplyr::rename(E1 = "sample(E1_series, 15)", E2 = "sample(E2_series, 15)") 
...
red_spp2 <- red_spp %>% dplyr::filter(sp != "s2") # removing sp2 because of extreme values
# from long to wide
rdiv_sim <- red_spp2 %>%
  spread( sp, dir_deriv)
...
rdiv_sim$rdiv<-apply(rdiv_sim[,c(4:6)], 1, resp_div, sign_sens = F)
rdiv_sim$sign<-apply(rdiv_sim[,c(4:6)], 1, resp_div, sign_sens = T)
...
(ref:RD-independent-plot) Directional derivatives and response diversity with known direction of env change. E1 and E2 change independently over time.
a. Species directional derivatives over time. b. Response diversity measured as similarity-based diversity metric. c. Response diversity measured as divergence (sign sensitive). 


```{r RD-independent-plot, fig.cap='(ref:RD-independent-plot)', fig.align="center", fig.height=6, fig.width=6}

dd_plot <- ggplot(data = red_spp2, mapping = aes(x = time, y = (dir_deriv), col = sp)) +
  geom_line()+
  theme_classic() +
   labs(x = "time",y = "Directional derivative", tag = "a")

rdiv_plot <- ggplot(data = rdiv_sim, mapping = aes(x = time, y = rdiv)) +
  geom_line()+
  theme_classic() +
   labs(x = "time",y = "Dissimilarity (derivatives)", tag = "b")


sing_plot <- ggplot(data = rdiv_sim, mapping = aes(x = time, y = sign)) +
  geom_line()+
  theme_classic() +
   labs(x = "time",y = "Divergence", tag = "c")
random_ev_change <- dd_plot / rdiv_plot / sing_plot
random_ev_change
```
...
refs_neg <- mvrnorm(n = 15,        #  specifies the sample size
                 ...
                 Sigma = matrix(c( 1, -0.3,
                                  -0.6,  1), #  specifies the correlation matrix
                                nrow = 2))
...
(ref:RD-negative-plot) Directional derivatives and response diversity with known direction of env change. E1 and E2 change with negative correlation over time.
a. Species directional derivatives over time. b. Response diversity measured as similarity-based diversity metric. c. Response diversity measured as divergence (sign sensitive). 


```{r RD-negative-plot, fig.cap='(ref:RD-negative-plot)', fig.align="center", fig.height=6, fig.width=6}
red_spp2[is.na(red_spp2)] <- 0
...
sing_plot <- ggplot(data = rdiv_sim, mapping = aes(x = time, y = sign)) +
  ...
  theme_classic() +
   labs(x = "time",y = "Divergence", tag = "c")
neg_ev_change <- dd_plot / rdiv_plot / sing_plot
neg_ev_change
```
...
(ref:RD-positive-plot) Directional derivatives and response diversity with known direction of env change. E1 and E2 change with negative correlation over time.
a. Species directional derivatives over time. b. Response diversity measured as similarity-based diversity metric. c. Response diversity measured as divergence (sign sensitive). 


```{r RD-positive-plot, fig.cap='(ref:RD-positive-plot)', fig.align="center", fig.height=6, fig.width=6}
red_spp2[is.na(red_spp2)] <- 0
...
(ref:ciliates-fig) Species responses to the environmental drivers.
a. Species responses to nutrient concentrations. b. Species responses to light intensity. 

```{r ciliates, fig.cap='(ref:ciliates)', fig.align="center", fig.height=5, fig.width=12}
...
nested_gams <- dd[,c(3:6)] %>% dplyr::rename(E1 = "PPM_conc",
  ...
  nest(cols =-species) %>% 
  mutate(
    gams = map(cols, ~ gam(density ~ ti(E1, k = 10) + ti(E2, k = 4) + te(E1, E2, k = 4),
                           data = .x,
                           method = "REML")),
    predicted = map(gams, ~ predict(.x, newdata = new_data))
  )
...
# Creating the datsaet with the 2 columns as described above
...
(ref:surface-fig) Response surface fitted with GAM. High non-linearity. Need to understand how to fix axis' values!

```{r surface-fig, fig.cap='(ref:surface-fig)', fig.align="center", fig.height=5, fig.width=12}
...
(ref:partialderiv-empirical) Response surface of Stylonychia. The two solid lines show at which level of nutrients and light each partial derivative is going to be calculated. Not sure we get the gray areas...

```{r partialderiv-empirical, fig.cap='(ref:partialderiv-empirical)', fig.align="center", fig.height=4, fig.width=6}
detach("package:MASS", unload=TRUE)
Stylonychia2 <- filter(nested_gams, species == "Stylonychia2")

E1 = as.numeric(unique(dd$PPM_conc))
E2 = as.numeric(unique(gsub("%","",as.character(dd$light))))
refs <- tibble(crossing(E1, E2))
m_Stylonychia2 <- (Stylonychia2$gams[[1]])
my_sp_names <- (Stylonychia2$species)
...
# evaluate te(x,z) at values of x & z
sm_E1 <- smooth_estimates(m_Stylonychia2, smooth = "te(E1,E2)", data = ds_E1) |>
  add_confint()


# partial derivatives
pd_E1 <- partial_derivatives(m_Stylonychia2, data = ds_E1, type = "central", focal = "E1")

# draw te(x,z)
p1 <- draw(m_Stylonychia2, rug = FALSE) &
...
(ref:partialeffectE1-empirical) Partial effect of nutrient concentration on the growth rate of Stylonychi when light intensity is held constant at 5.
...
(ref:partialderivE2-empirical) Response surface of Stylonychia. The two solid lines show at which level of nutrients and light each partial derivative is going to be calculated. Not sure we get the gray areas...
...
# evaluate te(x,z) at values of x & z
sm_E2 <- smooth_estimates(m_Stylonychia2, smooth = "te(E1,E2)", data = ds_E2) |>
  add_confint()


# partial derivatives
pd_E2 <- partial_derivatives(m_Stylonychia2, data = ds_E2, type = "central", focal = "E2")



# draw te(x,z)
p4 <- draw(m_Stylonychia2, rug = FALSE) &
  geom_hline(yintercept = E2_p, size = 1) &
  geom_vline(xintercept = E1_p, size = 1)
p4
```
...
(ref:partialeffectE2-empirical) Partial effect of light intensity on the growth rate of Stylonychi when nutrient concentration is held constant at 2.67.
...
(ref:summary-empirical) Summary plot of Stylonychia. (a) response surface of Stylonychia. (b) Partial effect of nutrient concentration on the density of Stylonychia when light intensity is held constant at 5. (c) Partial derivative with respect to nutrient concentration when light intensity is held constant at 5. (d) Partial effect of light intensity on the growth rate of Stylonychia when nutrient concentration is held constant at 2.67. (e) Partial derivative with respect to light intensity when nutrient concentration is held constant at 2.67.
...
To calculate the directional derivatives for all spp used in the experiment, we first create a time series with nutrient concentration and light intensity changing randomly over time, and then we fit GAMs individually for each species, and calculate partial derivatives.
```{r, echo=FALSE, results='hide'}
...
(ref:RD-empirical) Time series of (a) nutrient concentration and (b) light intensity changing over time.

```{r RD-empirical, fig.cap='(ref:RD-empirical)', fig.align="center", fig.height=3, fig.width=8}
...
(ref:RD-empirical-plot) Directional derivatives and response diversity with known direction of env change. 
a. Species directional derivatives over time. b. Response diversity measured as similarity-based diversity metric. c. Response diversity measured as divergence (sign sensitive). 


```{r RD-empirical-plot, fig.cap='(ref:RD-empirical-plot)', fig.align="center", fig.height=12, fig.width=10}
...
(ref:RD-compositions) Directional derivatives and response diversity with known direction of env change for three different communities. 
a. Species directional derivatives over time. b. Response diversity measured as similarity-based diversity metric. c. Response diversity measured as divergence (sign sensitive). 


```{r RD-compositions, fig.cap='(ref:RD-compositions)', fig.align="center", fig.height=12, fig.width=20}
