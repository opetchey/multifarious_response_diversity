...
p3 <- pd_E1 |>
    draw() &
    labs(x = "Temperature", y = "Partial derivative with respest to Temperature", title = "", caption = cap, tag = "(c)") & theme_classic(base_size = 20)  &
...
p4 <- draw(m1, rug = FALSE) &
  ...
  labs(tag = "(d)") & xlab("Temperature (k)") & ylab("Salinity (ppt)") & 
   labs(title = "", fill = "Growth", caption = "") & theme_classic(base_size = 20) 
p4 <- p4+ guides(fill=guide_legend(title="Growth \n Rate"))
...
p5<- sm_E2 |>
  ...
  labs(x = "Salinity", y = "Partial effect Salinity", title = "",
       caption = cap_E2, tag = "(e)") + theme_classic(base_size = 20)  
p5
...
p6 <- pd_E2 |>
  draw() +
  labs(x = "Salinity", y = "Partial derivative with respest to Salinity", title = "", caption = cap, tag = "(f)") & theme_classic(base_size = 20)  &
   geom_hline(yintercept = 0, size = 1, linetype = "dashed")
...
p_partials <- wrap_plots(p1E1, p2, p3,p4, p5, p6)
p_partials
ggsave(p_partials, file="/Users/francesco/Documents/GitHub/multifarious_response_diversity/Partials.jpg", width=15, height=8)
```
...
p4 <- draw(m1, rug = FALSE) &
  ...
  labs(tag = "(d)") & xlab("Temperature (k)") & ylab("Salinity (ppt)") & 
   labs(title = "", fill = "Growth", caption = "") & theme_classic(base_size = 20) 
p4 <- p4+ labs(tag = "(a)") + guides(fill=guide_legend(title="Growth \n Rate"))


fig6 <- p4 +  geom_segment(data = dd1,
               ...
               size=1) +  scale_colour_gradient2(low = muted("blue"),
                         ...
                         midpoint = 0)
fig6 
ggsave(fig6, file="/Users/francesco/Documents/GitHub/multifarious_response_diversity/one_point_unknown.jpg", width=10, height=8)
...
correlation_direction_env_change <- indipendent_fluctuations/negative_fluctuations/positive_fluctuations
ggsave(correlation_direction_env_change, file="/Users/francesco/Documents/GitHub/multifarious_response_diversity/correlation_direction_env_change.pdf", width=6, height=8)
